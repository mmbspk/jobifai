...
func NewTailor(client *llm.Client) *Tailor { return &Tailor{client: client} }
...
func (t *Tailor) TailorProfile(ctx context.Context, profile *domain.ResumeProfile, jobDesc string) (*domain.ResumeProfile, error) {
	...
	raw, err := t.client.Chat(ctx, []llm.Message{
...
func (t *Tailor) WriteCoverLetter(ctx context.Context, profile *domain.ResumeProfile, jobDesc string) (string, error) {
	...
	body, err := t.client.Chat(ctx, []llm.Message{
		{Role: "user", Content: prompt.String()},
	})
	if err != nil {
...
func (t *Tailor) AnswerFormQuestion(ctx context.Context, profile *domain.ResumeProfile, question string, options []string) (string, error) {
	profileJSON, err := json.Marshal(profile)
	if err != nil {
		return "", fmt.Errorf("form answer: marshal profile: %w", err)
	}
...
- Use application_defaults fields (requires_sponsorship, notice_period, salary_expectation) when relevant`,
	...
	answer, err := t.client.Chat(ctx, []llm.Message{{Role: "user", Content: prompt}})
	if err != nil {
